...
public class IndexRecord extends IndexDocument {
    ...
    @JsonProperty("licenseObject")
    List<HashMap<String, String>> license = new ArrayList<>();

    @JsonProperty("MD_LegalConstraintsOtherConstraintsObject")
    List<HashMap<String, String>> MD_LegalConstraintsOtherConstraints = new ArrayList<>();
    @JsonProperty("MD_ConstraintsOtherConstraintsObject")
    List<HashMap<String, String>> MD_ConstraintsOtherConstraints = new ArrayList<>();
    @JsonProperty("MD_SecurityConstraintsOtherConstraintsObject")
    List<HashMap<String, String>> MD_SecurityConstraintsOtherConstraints = new ArrayList<>();

    @JsonProperty("MD_LegalConstraintsUseLimitationObject")
    List<HashMap<String, String>> MD_LegalConstraintsUseLimitation = new ArrayList<>();
    @JsonProperty("MD_ConstraintsUseLimitationObject")
    List<HashMap<String, String>> MD_ConstraintsUseLimitation = new ArrayList<>();
    @JsonProperty("MD_SecurityConstraintsUseLimitationObject")
    List<HashMap<String, String>> MD_SecurityConstraintsUseLimitation = new ArrayList<>();
    ...
    @JsonProperty(IndexRecordFieldNames.isPublishedToAll)
    ...
    private List<ResourceIdentifier> resourceIdentifier = new ArrayList<>();
    ...
    //  @JsonAnyGetter
    private Map<String, WrappedStringList> otherProperties = new HashMap<>();

    private Map<String, WrappedCodelistList> codelists = new HashMap<>();

    @JsonAnySetter
    public void addOtherProperties(String key, WrappedStringList value) {
        otherProperties.put(key, value);
    }

    @JsonAnyGetter
    public Map<String, WrappedStringList> getOtherProperties() {
    ...
    public IndexRecord(Metadata r) {
        ...
        this.setSchema(r.getSchemaId());
        this.setDocType(IndexDocumentType.metadata);
        this.setDraft(isDraft ? "y" : "n");
        this.setIsTemplate(r.getIsTemplate().charAt(0));
        this.setDocumentStandard(r.getSchemaId());
        this.setCreateDate(r.getCreateDate()); // .getDateAndTime()
        this.setChangeDate(r.getChangeDate()); // .getDateAndTime()

        this.setOwner(r.getOwner());
        this.setGroupOwner(r.getGroupOwner());
        ...
        this.setHarvested(r.getIsHarvested().equalsIgnoreCase("y"));
        this.setHarvesterUuid(r.getHarvestUuid());
        // TODO: Review: only for harvested metadata
        this.setSourceCatalogue(r.getHarvestUuid());
        /*this.setLogo("");*/
        this.setPublishedToAll(true);

        this.setPopularity(r.getPopularity());
    ...
    public void updateInfo(Metadata r) {
        r.getPrivileges().forEach(p -> {
            WrappedStringList l = getOtherProperties().get("op" + p.getId().getOperationId());

            if (l == null) {
                l = new WrappedStringList();
            }

            l.getWrapped().add(String.valueOf(p.getId().getGroupId()));
            addOtherProperties("op" + p.getId().getOperationId(), l);
        });
    }
    /**
    ...
    public void ignored(String name, Object value) {
        ...
        try {
            ...
            WrappedStringList s = otherProperties.get(name);
            if (s == null) {
                s = new WrappedStringList();

                if (value instanceof Codelist) {
                    s.getWrapped().add(value);
                } else {
                    s.getWrapped().add(value.toString());
                }
